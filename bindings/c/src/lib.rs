//! The wickra-verify C ABI — the hub every C-capable language links against.
//!
//! The surface is deliberately tiny and JSON-shaped, exactly like
//! `Verifier::command_json`: create a verifier handle, drive it with command
//! JSONs (`verify`, `explain`, `canonicalize`, `version`) and read back response
//! JSONs, then free the handle. No verdict type crosses the boundary by value —
//! the handle is opaque and the payloads are always UTF-8 JSON strings.
//!
...
//!
//! 1. Call [`wickra_verify_command`] with `out = NULL`, `cap = 0` to learn the
...
//! for unusable arguments ([`WICKRA_VERIFY_ERR_NULL`],
//! [`WICKRA_VERIFY_ERR_UTF8`]) and caught panics ([`WICKRA_VERIFY_ERR_PANIC`]);
//! a non-negative return is always the response length. Domain errors (a bad
//! claim, an unknown command) are *not* negative — they come back in-band as
...
pub const WICKRA_VERIFY_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_VERIFY_ERR_PANIC: i32 = -3;

/// An opaque handle to a verifier instance. Created by [`wickra_verify_new`] and
/// destroyed by [`wickra_verify_free`]; never dereferenced by the caller.
pub struct WickraVerify(Verifier);
...
/// Construct a verifier handle with the default tolerances.
///
/// Never fails; free the handle with [`wickra_verify_free`].
#[no_mangle]
pub extern "C" fn wickra_verify_new() -> *mut WickraVerify {
    Box::into_raw(Box::new(WickraVerify(Verifier::default())))
}

/// Destroy a verifier handle. Null is a no-op.
///
/// # Safety
/// `handle` must be null or a handle previously returned by
/// [`wickra_verify_new`] and not already freed.
#[no_mangle]
pub unsafe extern "C" fn wickra_verify_free(handle: *mut WickraVerify) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_verify_command(
    handle: *mut WickraVerify,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_VERIFY_ERR_NULL;
    }
    let Some(cmd) = (unsafe { opt_str(cmd_json) }) else {
        return WICKRA_VERIFY_ERR_UTF8;
    };
    let verifier = unsafe { &(*handle).0 };
    let response = match catch_unwind(AssertUnwindSafe(|| verifier.command_json(cmd))) {
        ...
        Err(_) => return WICKRA_VERIFY_ERR_PANIC,
    };
...
#[no_mangle]
pub extern "C" fn wickra_verify_version() -> *const c_char {
...
mod tests {
    ...
    fn new_command_free_round_trip() {
        ...
        unsafe { wickra_verify_free(handle) };
    }
    ...
    fn bad_command_reports_error_in_band() {
        ...
        let len = unsafe { wickra_verify_command(handle, bad.as_ptr(), ptr::null_mut(), 0) };
    ...
    fn null_guards_on_command() {
        ...
        // Null handle.
        let code =
            unsafe { wickra_verify_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_VERIFY_ERR_NULL);
        // Null command with a valid handle.
        let handle = wickra_verify_new();
        let code = unsafe { wickra_verify_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_VERIFY_ERR_NULL);
        unsafe { wickra_verify_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_verify_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_verify_version();
