...
/** Raw FFM (Panama) downcall surface for the wickra-verify C ABI. */
final class Native {
    private Native() {}
    ...
    static final MethodHandle NEW =
            handle("wickra_verify_new", FunctionDescriptor.of(C_PTR));
    static final MethodHandle FREE =
            handle("wickra_verify_free", FunctionDescriptor.ofVoid(C_PTR));
    static final MethodHandle COMMAND =
            handle("wickra_verify_command", FunctionDescriptor.of(C_INT, C_PTR, C_PTR, C_PTR, C_LONG));
    static final MethodHandle VERSION =
            handle("wickra_verify_version", FunctionDescriptor.of(C_PTR));
    ...
    private static SymbolLookup loadLibrary() {
        String libFile = System.mapLibraryName("wickra_verify");
        ...
        throw new UnsatisfiedLinkError("wickra-verify: could not load the native library (" + libFile
    ...
    private static Path extractBundled(String libFile) {
        ...
        try (InputStream in = Native.class.getResourceAsStream(resource)) {
            ...
            Path tmp = Files.createTempFile("wickra-verify-", "-" + libFile);
